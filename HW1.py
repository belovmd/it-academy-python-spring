...
def solve(n):
    ...
    return [solution + [(n, i + 1)]
            ...
            if not under_attack(g + 1, solution)]
...
def validate(queensss):
    left = right = col = queensss[-1]
    for r in reversed(queensss[:-1]):
        left, right = left - 1, right + 1
        if r in (left, col, right):
            raise BailOut


def add_queen(queensss, queens):
    for y in range(BOARD_SIZE):
        test_queens = queens + [y]
        try:
